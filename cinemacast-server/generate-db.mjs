import * as fs from "fs";

const movies = [
  {
    id: 1,
    imdbID: "tt10872600",
    title: "Spider-Man: No Way Home",
    year: 2021,
    director: "Jon Watts",
    genre: "Action",
    rating: 8.4,
    budgetInMillions: 200,
  },
  {
    id: 2,
    imdbID: "tt1160419",
    title: "Dune",
    year: 2021,
    director: "Denis Villeneuve",
    genre: "Sci-Fi",
    rating: 8.1,
    budgetInMillions: 165,
  },
  {
    id: 3,
    imdbID: "tt1877830",
    title: "The Batman",
    year: 2022,
    director: "Matt Reeves",
    genre: "Action",
    rating: 8.3,
    budgetInMillions: 185,
  },
  {
    id: 4,
    imdbID: "tt2382320",
    title: "No Time to Die",
    year: 2021,
    director: "Cary Joji Fukunaga",
    genre: "Action",
    rating: 7.3,
    budgetInMillions: 250,
  },
  {
    id: 5,
    imdbID: "tt10838180",
    title: "The Matrix Resurrections",
    year: 2021,
    director: "Lana Wachowski",
    genre: "Sci-Fi",
    rating: 5.7,
    budgetInMillions: 190,
  },
  {
    id: 6,
    imdbID: "tt9376612",
    title: "Shang-Chi and the Legend of the Ten Rings",
    year: 2021,
    director: "Destin Daniel Cretton",
    genre: "Action",
    rating: 7.4,
    budgetInMillions: 150,
  },
  {
    id: 7,
    imdbID: "tt2953050",
    title: "Encanto",
    year: 2021,
    director: "Byron Howard, Jared Bush",
    genre: "Animation",
    rating: 7.9,
    budgetInMillions: 120,
  },
  {
    id: 8,
    imdbID: "tt8847712",
    title: "The French Dispatch",
    year: 2021,
    director: "Wes Anderson",
    genre: "Comedy",
    rating: 7.2,
    budgetInMillions: 25,
  },
  {
    id: 9,
    imdbID: "tt11286314",
    title: "Don't Look Up",
    year: 2021,
    director: "Adam McKay",
    genre: "Comedy",
    rating: 7.2,
    budgetInMillions: 75,
  },
  {
    id: 10,
    imdbID: "tt6334354",
    title: "The Suicide Squad",
    year: 2021,
    director: "James Gunn",
    genre: "Action",
    rating: 7.2,
    budgetInMillions: 185,
  },
  {
    id: 11,
    imdbID: "tt6264654",
    title: "Free Guy",
    year: 2021,
    director: "Shawn Levy",
    genre: "Comedy",
    rating: 7.1,
    budgetInMillions: 100,
  },
  {
    id: 12,
    imdbID: "tt3228774",
    title: "Cruella",
    year: 2021,
    director: "Craig Gillespie",
    genre: "Comedy",
    rating: 7.4,
    budgetInMillions: 100,
  },
  {
    id: 13,
    imdbID: "tt9032400",
    title: "Eternals",
    year: 2021,
    director: "Chloé Zhao",
    genre: "Action",
    rating: 6.4,
    budgetInMillions: 200,
  },
  {
    id: 14,
    imdbID: "tt8332922",
    title: "A Quiet Place Part II",
    year: 2021,
    director: "John Krasinski",
    genre: "Horror",
    rating: 7.3,
    budgetInMillions: 61,
  },
  {
    id: 15,
    imdbID: "tt3480822",
    title: "Black Widow",
    year: 2021,
    director: "Cate Shortland",
    genre: "Action",
    rating: 6.7,
    budgetInMillions: 200,
  },
  {
    id: 16,
    imdbID: "tt0870154",
    title: "Jungle Cruise",
    year: 2021,
    director: "Jaume Collet-Serra",
    genre: "Adventure",
    rating: 6.6,
    budgetInMillions: 200,
  },
  {
    id: 17,
    imdbID: "tt4513678",
    title: "Ghostbusters: Afterlife",
    year: 2021,
    director: "Jason Reitman",
    genre: "Comedy",
    rating: 7.1,
    budgetInMillions: 75,
  },
  {
    id: 18,
    imdbID: "tt12801262",
    title: "Luca",
    year: 2021,
    director: "Enrico Casarosa",
    genre: "Animation",
    rating: 7.5,
    budgetInMillions: 50,
  },
  {
    id: 19,
    imdbID: "tt1321510",
    title: "In the Heights",
    year: 2021,
    director: "Jon M. Chu",
    genre: "Drama",
    rating: 7.3,
    budgetInMillions: 55,
  },
  {
    id: 20,
    imdbID: "tt11214590",
    title: "House of Gucci",
    year: 2021,
    director: "Ridley Scott",
    genre: "Drama",
    rating: 6.6,
    budgetInMillions: 75,
  },
];

const actors = [
  {
    id: 1,
    imdbID: "nm4043618",
    name: "Tom Holland",
  },
  {
    id: 2,
    imdbID: "nm1083271",
    name: "Zendaya",
  },
  {
    id: 3,
    imdbID: "nm0000138",
    name: "Timothée Chalamet",
  },
  {
    id: 4,
    imdbID: "nm0001876",
    name: "Rebecca Ferguson",
  },
  {
    id: 5,
    imdbID: "nm2394794",
    name: "Robert Pattinson",
  },
  {
    id: 6,
    imdbID: "nm1226817",
    name: "Zoë Kravitz",
  },
  {
    id: 7,
    imdbID: "nm0000296",
    name: "Daniel Craig",
  },
  {
    id: 8,
    imdbID: "nm0654110",
    name: "Rami Malek",
  },
  {
    id: 9,
    imdbID: "nm0908094",
    name: "Keanu Reeves",
  },
  {
    id: 10,
    imdbID: "nm0000246",
    name: "Carrie-Anne Moss",
  },
  {
    id: 11,
    imdbID: "nm5124257",
    name: "Simu Liu",
  },
  {
    id: 12,
    imdbID: "nm1128026",
    name: "Awkwafina",
  },
  {
    id: 13,
    imdbID: "nm0662722",
    name: "Stephanie Beatriz",
  },
  {
    id: 14,
    imdbID: "nm1832688",
    name: "Lin-Manuel Miranda",
  },
  {
    id: 15,
    imdbID: "nm0005562",
    name: "Benicio Del Toro",
  },
  {
    id: 16,
    imdbID: "nm1172478",
    name: "Adrien Brody",
  },
  {
    id: 17,
    imdbID: "nm0000093",
    name: "Leonardo DiCaprio",
  },
  {
    id: 18,
    imdbID: "nm0000192",
    name: "Jennifer Lawrence",
  },
  {
    id: 19,
    imdbID: "nm1641117",
    name: "Margot Robbie",
  },
  {
    id: 20,
    imdbID: "nm0005377",
    name: "Idris Elba",
  },
  // Add more actors as needed
];

const credits = [
  {
    id: 1,
    movieId: 1,
    actorId: 1,
    role: "Peter Parker / Spider-Man",
  },
  {
    id: 2,
    movieId: 1,
    actorId: 2,
    role: "MJ",
  },
  {
    id: 3,
    movieId: 2,
    actorId: 3,
    role: "Paul Atreides",
  },
  {
    id: 4,
    movieId: 2,
    actorId: 4,
    role: "Lady Jessica",
  },
  {
    id: 5,
    movieId: 3,
    actorId: 5,
    role: "Bruce Wayne / Batman",
  },
  {
    id: 6,
    movieId: 3,
    actorId: 6,
    role: "Selina Kyle / Catwoman",
  },
  {
    id: 7,
    movieId: 4,
    actorId: 7,
    role: "James Bond",
  },
  {
    id: 8,
    movieId: 4,
    actorId: 8,
    role: "Lyutsifer Safin",
  },
  {
    id: 9,
    movieId: 5,
    actorId: 9,
    role: "Neo",
  },
  {
    id: 10,
    movieId: 5,
    actorId: 10,
    role: "Trinity",
  },
  {
    id: 11,
    movieId: 6,
    actorId: 11,
    role: "Shang-Chi",
  },
  {
    id: 12,
    movieId: 6,
    actorId: 12,
    role: "Katy",
  },
  {
    id: 13,
    movieId: 7,
    actorId: 13,
    role: "Mirabel",
  },
  {
    id: 14,
    movieId: 7,
    actorId: 14,
    role: "Bruno",
  },
  {
    id: 15,
    movieId: 8,
    actorId: 15,
    role: "Moses Rosenthaler",
  },
  {
    id: 16,
    movieId: 8,
    actorId: 16,
    role: "Julian Cadazio",
  },
  {
    id: 17,
    movieId: 9,
    actorId: 17,
    role: "Dr. Randall Mindy",
  },
  {
    id: 18,
    movieId: 9,
    actorId: 18,
    role: "Kate Dibiasky",
  },
  {
    id: 19,
    movieId: 10,
    actorId: 19,
    role: "Harley Quinn",
  },
  {
    id: 20,
    movieId: 10,
    actorId: 20,
    role: "Bloodsport",
  },
  // Add more credits as needed
];

// console.log(credits);

let movieDb = {
  movies,
  actors,
  credits,
};

// console.log(movieDb);

const dbJSON = JSON.stringify(movieDb);

let fileName = "./db.json";
fs.writeFile(fileName, dbJSON, function (err, data) {
  if (err) console.log(err);
  console.log(`Successfully wrote db.json file to: ${fileName}`);
});
